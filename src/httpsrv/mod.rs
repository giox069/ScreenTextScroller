...
fn append_session_cookie_to_response(response: &mut Response<Body>, sid: String) {
    ...
    response
        .headers_mut()
        .insert("Set-Cookie", HeaderValue::from(cookie_header));
}

async fn serve_authservice(
    d: Arc<Mutex<LpfHttpServerData>>,
    req: Request<Body>,
...
async fn serve_logoff(
    ...
    *response.body_mut() = Body::from(format!("Redirecting to {}", AUTHENTICATION_PAGE));
    response
        .headers_mut()
        .insert("Location", HeaderValue::from_static(AUTHENTICATION_PAGE));
...
async fn serve_upload(
    ...
    let boundary = match req.headers().get(HEADER_CONTENT_TYPE).and_then(|ct| {
        let ct = ct.to_str().ok()?;
        ...
        Some(b) => b,
        None => {
            let r = json!({ "err": "Cannot find boundary" }).to_string();
            *response.body_mut() = Body::from(r);
            return;
        }
    };
    ...
    let bb = match hyper::body::to_bytes(req.into_body()).await {
        Ok(bb) => bb,
        Err(_e) => {
    ...
    let mp = Multipart::with_body(Cursor::new(bb), boundary);
    if let Entry(mut mpf) = mp.into_entry() {
        loop {
            if mpf.headers.content_type.is_some() && mpf.headers.filename.is_some() {
                let ld = d.lock().await;
                let rgd = ld.runtime_global_data.lock().await;
                let outdir = rgd.upload_dir.clone();
                println!(
                    "Saving background file {} to {:?}",
                    mpf.headers.filename.clone().unwrap(),
                    outdir
                );
                mpf.data
                    .save()
    ...
    let errj = json!({"err": "Upload ended... code in ptogress" });
    *response.body_mut() = Body::from(errj.to_string());
}
...
async fn serve_lapi(
    ...
    let lapi_result = match cmd {
        ...
        _ => {
            let r =
                json!({ "err": format!("{} is not recognized as a lapi cmd", cmd) }).to_string();
            *response.body_mut() = Body::from(r);
            return;
        }
    };
...
async fn serve_page(
        ...
        && p != AUTHENTICATION_PAGE
    {
        *response.body_mut() = Body::from(format!(
            "While serving static page, authentication is needed. Redirecting to {}",
            AUTHENTICATION_PAGE
        ));
        response
            .headers_mut()
            ...
            */
            ...
            if content_type == "text/html" {
                let mut cs = match str::from_utf8(&content) {
                    Ok(v) => v.to_string(),
                    Err(_e) => String::from(
                        "Unable to convert html file to UTF8 string. Invalid UTF-8 chars?",
                    ),
                };
                cs = cs
            ...
            response
                .headers_mut()
                .insert(HEADER_CONTENT_TYPE, HeaderValue::from_static(content_type));
...
pub async fn httpd_main(
    runtime_global_data: &Arc<Mutex<RuntimeGlobalData>>,
) -> Result<(), std::io::Error> {
    let cf = runtime_global_data.lock().await;

    let d = Arc::new(Mutex::new(LpfHttpServerData {
    ...
    let make_svc = make_service_fn(move |conn: &AddrStream| {
        ...
        async move {
            Ok::<_, Infallible>(service_fn(move |req: hyper::Request<Body>| {
                let addr = addr.clone();
                let d = d.clone();
                async move { Ok::<_, Infallible>(servicefn(d, req, addr.clone()).await) }
            }))
        }
    });
