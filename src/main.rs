...
impl MyWindowHandler {
    fn rebuild_text_block(&mut self) {
        ...
        let block = self.font.layout_text(
            &self.current_cfg_copy.disp_text,
...
impl WindowHandler for MyWindowHandler {
    fn on_draw(&mut self, helper: &mut WindowHelper, graphics: &mut Graphics2D) {
        ...
        if config_has_changed {
            self.y = match self.current_cfg_copy.disp_orientation {
                ...
                TextScrollOrientation::ScrollHorizontal => {
                    self.current_cfg_copy.disp_vmargin as f32
                }
            };
        ...
        let clip_area = Rectangle::from_tuples(
            (
                self.current_cfg_copy.disp_hmargin as i32,
                self.current_cfg_copy.disp_vmargin as i32,
            ),
            (
                self.size.x as i32 - self.current_cfg_copy.disp_hmargin as i32,
                self.size.y as i32 - self.current_cfg_copy.disp_vmargin as i32,
            ),
        );
        ...
        match &self.block {
            Some(b) => {
                ...
                graphics.draw_text((self.x, self.y), fgcolor, &b);
...
async fn main() {
    ...
    let upload_dir = runtime_data_dir.join("upload");
    let cfg: lconfig::Config = lconfig::Config::load().await;

    let _err = fs::create_dir(upload_dir.clone()).await;

    let runtime_global_data = Arc::new(Mutex::new(RuntimeGlobalData {
